...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmValidation = password !== confirmPassword ? 'Passwords do not match' : '';
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <Button type="submit" variant="contained" disabled={loading} sx={{ py: 1.5, borderRadius: '12px', fontWeight: 600 }}>
            {loading ? <CircularProgress size={20} sx={{ color: 'white' }} /> : 'Sign Up'}
          </Button>
        </form>

        <Typography sx={{ mt: 2, textAlign: 'center' }}>
          Already have an account? <Link to="/login" style={{ color: 'primary.main', fontWeight: 600 }}>Sign In</Link>
        </Typography>
      </Box>
    </Box>
  );
